...
def main():
    import random
    def assert_eq(x, y, more=''):
        if x != y:
            raise AssertionError(f'{x} != {y}\n{more}')
    def assert_avl(node):
        if node is None:
            return 0
        left_height = assert_avl(node.left)
        right_height = assert_avl(node.right)
        assert_eq(right_height - left_height, node.balance, node)
        return 1 + max(left_height, right_height)
    values = list(range(20,30))+list(range(10,20))[::-1]+random.sample(range(30,50),20)+list(range(50,60))
    print(f'Inserting {len(values)} values: {values}')
    tree = avl_tree()
    mirror = []
    cut = 0
    for i,v in enumerate(values):
        assert_eq(i, len(tree))
        index = 0
        while index < i and mirror[index] < v:
            index += 1
        print(f'Inserting {v} at {index} -> size {i+1}')
        mirror.insert(index, v)
        tree.insert(index, v)
        if i >= cut:
            print(tree)
            cut = int(i ** 1.5 - 1)
        assert_avl(tree.root)
        assert_eq(mirror, list(tree))
    print(tree)
    values = random.sample(range(30,50),20)+list(range(50,60))[::-1]+list(range(10,30))
    print(f'Deleting {len(values)} values: {values}')
    cut = 0
    for i,v in enumerate(values):
        assert_eq(len(mirror), len(tree))
        index = mirror.index(v)
        print(f'Deleting {v} at {index} -> size {len(mirror)-1}')
        assert_eq(v, mirror.pop(index))
        assert_eq(v, tree.delete(index))
        if i >= cut:
            print(tree)
            cut = int(i ** 1.5 - 1)
        assert_avl(tree.root)
        assert_eq(mirror, list(tree))
    assert_eq(0, len(tree))
    print(tree)
